...
Tocco lungo di tre secondi sul logo nella schermata di attesa, poi codice numerico (al primo accesso lo si sceglie; se si dimentica, si azzera da *Backup*). Sette schede: **Catalogo** (import dal gestionale, attivo/inattivo, referenze senza profilo in rosso), **Profili** (revisione, con le bozze incerte in cima), poi una scheda per porta — **Domande** (editor delle domande, *prova rapida* con risultati in tempo reale, coefficienti del punteggio), **Note** (soglie, prova e salute della *ricerca per note*), **Parole** (le parole che il lessico non conosce ancora, contate una per una e scaricabili; pesi e prova del *consulente a parole* con le frasi tipiche) —, **Statistiche** (anonime e aggregate, comprese le note più cercate, le scene più raccontate e da quale delle tre porte entra la clientela; le frasi dei clienti non si salvano mai), **Backup** (export/import, punti di ripristino, ritorno ai valori consigliati).
